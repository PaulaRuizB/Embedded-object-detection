...
def allocate_buffers_descriptor(engine, data_shape=None):
    inputs = []
    outputs = []
    bindings = []
    stream = cuda.Stream()
    for binding in engine:
        if engine.binding_is_input(binding):
            ...
            size = engine.get_binding_shape(binding)[1] * data_shape[0]
            #size = trt.volume(engine.get_binding_shape(binding))#estaba hasta aqui #* data.shape[0] #engine.max_batch_size
...
# Allocates all buffers required for an engine, i.e. host/device inputs/outputs.
def allocate_buffers(engine, data_shape=None):
    inputs = []
    outputs = []
    bindings = []
    stream = cuda.Stream()
    for binding in engine:
        if engine.binding_is_input(binding):
            ...
            size = trt.volume(engine.get_binding_shape(binding)) #* engine.max_batch_size #salta un warning si lo dejo porque da 1
